...
namespace Depra.Assets.Runtime.Bundle.Strategies.Memory
{
    public sealed class AssetBundleFromMemory : AssetFileSource<AssetBundle>
    {
        private readonly ICoroutineHost _coroutineHost;

        public AssetBundleFromMemory(ICoroutineHost coroutineHost) =>
        ...
        {
            var bytes = File.ReadAllBytes(assetFile.Path);
            var loadedAssetBundle = AssetBundle.LoadFromMemory(bytes);
